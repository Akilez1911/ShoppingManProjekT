...
public class Customer {
	public void addToCart(int p, int q,String email) throws Exception
	{
		...
		while(rs.next())
		{
			...
			String sql="Insert into cart values('"+prod_id+"', '"+s3+"', '"+price+"', '"+q+"','"+email+"')";
	...
	public void removeCart(int p,String email) throws Exception{
		db_connection.connection();
		String sql="Delete from cart where prod_id='"+p+"' and email='"+email+"'";
		Statement s=(Statement) db_connection.con.createStatement();
		int conf=s.executeUpdate(sql);
		if(conf==1)
	...
	public void bill(String email) throws Exception{
		db_connection.connection();
		String sql="Select * from cart where email='"+email+"'";
		Statement s=(Statement) db_connection.con.createStatement();
		ResultSet conf=s.executeQuery(sql);
		...
		if(f!=0)
		{
			String sql2="Select sum(price) as sum from cart where email='"+email+"'";
